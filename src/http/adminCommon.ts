...
function requireAdmin(req: PlatformRequest, res: Response, next: NextFunction) {
...
function ensureDbAuthProvider(res: Response) {
  if (isDbAuthProvider()) {
    return true;
  }
  res.status(400).json({
...
function parseAdminPayload(req: PlatformRequest, res: Response): Record<string, unknown> | null {
