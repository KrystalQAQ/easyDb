...
import { readActor, type PlatformRequest } from "./platform/types";
...
export function createPlatformApiRoutes() {
  ...
  router.use((req, res, next) => {
    const request = req as PlatformRequest;
    if (request.apiKeyContext) return next();
    return requireAdmin(request, res, next);
  });

  // 辅助：解析 projectEnvId，API Key 认证时同时校验 project/env 范围
  async function resolveEnvId(req: PlatformRequest, res: any) {
  ...
  router.post("/projects/:projectKey/envs/:env/api-groups", async (req, res) => {
    ...
    try {
      ...
      const actor = readActor(req as PlatformRequest);
      await writeAuditLog({
  ...
  router.get("/projects/:projectKey/envs/:env/apis", async (req, res) => {
    ...
    if (!ctx) return;
    try {
      const options: { groupId?: number; status?: string } = {};
      const groupKeyQuery = req.query.groupKey;
      if (typeof groupKeyQuery === "string" && groupKeyQuery.trim()) {
        const group = await getApiGroup(ctx.envId, normalizeGroupKey(groupKeyQuery));
  ...
  router.put("/projects/:projectKey/envs/:env/apis/:apiKey", async (req, res) => {
    ...
    try {
      const actor = readActor(req as PlatformRequest);
      const data = { ...payload, actor: actor.username };
