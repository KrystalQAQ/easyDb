...
const nginxProjectFrontendWebRootTemplate = (
  process.env.NGINX_PROJECT_FRONTEND_WEB_ROOT_TEMPLATE || "/app/runtime/project-web/{projectKey}/{env}/current"
).trim();
const nginxPathBasePrefix = normalizePathPrefix(process.env.NGINX_PATH_BASE_PREFIX || "/p", "/p");
const nginxPathServerName = (process.env.NGINX_PATH_SERVER_NAME || "_").trim() || "_";
const nginxPathRoutingConfFileName = (process.env.NGINX_PATH_ROUTING_CONF_FILENAME || "easydb_path_routing.conf").trim();
const bootstrapDbConfig = resolveBootstrapDbConfig();

module.exports = {
  port: parseNumber(process.env.PORT, 3000),
  db: {
    host: process.env.DB_HOST || bootstrapDbConfig?.host || "",
    port: parseNumber(process.env.DB_PORT, bootstrapDbConfig?.port || 3306),
    user: process.env.DB_USER || bootstrapDbConfig?.user || "",
    password:
      process.env.DB_PASSWORD !== undefined
        ? process.env.DB_PASSWORD
        : bootstrapDbConfig?.password || "",
    database: process.env.DB_NAME || bootstrapDbConfig?.database || "",
  },
  ...
  authProvider: (process.env.AUTH_PROVIDER || "db").toLowerCase(),
  ...
  platform: {
    ...
    defaultEnv: {
      ...
      db: {
        host:
          process.env.PLATFORM_DEFAULT_DB_HOST ||
          process.env.DB_HOST ||
          bootstrapDbConfig?.host ||
          "",
        port: parseNumber(
          process.env.PLATFORM_DEFAULT_DB_PORT,
          parseNumber(process.env.DB_PORT, bootstrapDbConfig?.port || 3306)
        ),
        user: process.env.PLATFORM_DEFAULT_DB_USER || process.env.DB_USER || bootstrapDbConfig?.user || "",
        password:
  ...
  nginx: {
    ...
    frontendRoot: process.env.NGINX_FRONTEND_ROOT || "/app/frontend-app/dist",
    ...
    projectFrontendWebRootTemplate:
      nginxProjectFrontendWebRootTemplate || "/app/runtime/project-web/{projectKey}/{env}/current",
    upstreamOrigin: process.env.NGINX_UPSTREAM_ORIGIN || "http://127.0.0.1:3000",
    pathRoutingEnabled: parseBoolean(process.env.NGINX_PATH_ROUTING_ENABLED, true),
    pathBasePrefix: nginxPathBasePrefix,
    pathServerName: nginxPathServerName,
    pathRoutingConfFileName: nginxPathRoutingConfFileName || "easydb_path_routing.conf",
    reloadCommand: process.env.NGINX_RELOAD_COMMAND || "nginx -s reload",
  },
