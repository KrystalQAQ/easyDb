...
function renderSqlTemplate(template: string, params: Record<string, any>) {
  const values: any[] = [];
...
function validateApiParams(rawParams: any, paramsSchema: any[]) {
...
function applyResultMapping(rawResult: any, mapping: any) {
  ...
  if (mapping.fields && typeof mapping.fields === "object") {
    const remap = (row: Record<string, any>) => {
      const out: Record<string, any> = {};
      for (const [alias, col] of Object.entries(mapping.fields)) {
        out[alias] = row[String(col)];
      }
      return out;
    };
...
async function executeApiRequest(req: any, res: any, options: any = {}) {
