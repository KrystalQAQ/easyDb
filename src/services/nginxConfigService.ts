...
function normalizePathBasePrefix(value) {
  const text = String(value || nginx.pathBasePrefix || "/p").trim();
  if (!text.startsWith("/")) {
    throw new Error("pathBasePrefix must start with /");
  }
  const normalized = text === "/" ? "/" : text.replace(/\/+$/, "");
  if (!normalized || normalized === "/") {
    throw new Error("pathBasePrefix must not be root /");
  }
  return normalized;
}

function resolveSharedPathRoutingConfPath() {
  const fileName = normalizeFileName(nginx.pathRoutingConfFileName || "easydb_path_routing.conf");
  return path.join(nginx.confDir, fileName);
}

function buildProjectPathBase(projectKey, env) {
  const normalizedProject = normalizeKeySegment(projectKey, "projectKey");
  const normalizedEnv = normalizeKeySegment(env, "env");
  const basePrefix = normalizePathBasePrefix(nginx.pathBasePrefix);
  return `${basePrefix}/${encodeURIComponent(normalizedProject)}/${encodeURIComponent(normalizedEnv)}`;
}
...
function buildNginxConfigText(options) {
  ...
  const pathBase = buildProjectPathBase(projectKey, env);
  ...
  return [
    ...
    `    proxy_pass ${upstreamOrigin}/api/auth/authorize;`,
    "    proxy_http_version 1.1;",
    "    proxy_set_header Host $host;",
    "    proxy_set_header X-Real-IP $remote_addr;",
    "    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;",
    ...
    `    proxy_pass ${upstreamOrigin}/api/auth/token;`,
    "    proxy_http_version 1.1;",
    "    proxy_set_header Host $host;",
    "    proxy_set_header X-Real-IP $remote_addr;",
    "    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;",
...
function buildSharedPathRoutingConfigText() {
  const listenPort = normalizeListenPort(nginx.listenPort);
  const upstreamOrigin = normalizeUpstreamOrigin(nginx.upstreamOrigin);
  const serverName = String(nginx.pathServerName || "_").trim() || "_";
  const pathBasePrefix = normalizePathBasePrefix(nginx.pathBasePrefix);
  const webRootTemplate = normalizeFrontendRoot(nginx.projectFrontendWebRootTemplate);
  const escapedPrefix = pathBasePrefix.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  ...
  return [
    ...
    "  charset utf-8;",
    "",
    `  # 内网共享入口：${pathBasePrefix}/{projectKey}/{env}/`,
    `  location ~ ^${escapedPrefix}/(?<projectKey>[a-z0-9_-]+)/(?<env>[a-z0-9_-]+)$ {`,
    `    return 302 ${pathBasePrefix}/$projectKey/$env/;`,
    "  }",
    "",
    "  # 平台登录接口（全局）",
    `  location ~ ^${escapedPrefix}/(?<projectKey>[a-z0-9_-]+)/(?<env>[a-z0-9_-]+)/api/auth/login$ {`,
    `    proxy_pass ${upstreamOrigin}/api/auth/login;`,
    "    proxy_http_version 1.1;",
    "    proxy_set_header Host $host;",
    "    proxy_set_header X-Real-IP $remote_addr;",
    ...
    "  }",
    "",
    `  location ~ ^${escapedPrefix}/(?<projectKey>[a-z0-9_-]+)/(?<env>[a-z0-9_-]+)/api/auth/authorize$ {`,
    `    proxy_pass ${upstreamOrigin}/api/auth/authorize;`,
    "    proxy_http_version 1.1;",
    "    proxy_set_header Host $host;",
    "    proxy_set_header X-Real-IP $remote_addr;",
    ...
    "  }",
    "",
    `  location ~ ^${escapedPrefix}/(?<projectKey>[a-z0-9_-]+)/(?<env>[a-z0-9_-]+)/api/auth/token$ {`,
    `    proxy_pass ${upstreamOrigin}/api/auth/token;`,
    "    proxy_http_version 1.1;",
    "    proxy_set_header Host $host;",
    "    proxy_set_header X-Real-IP $remote_addr;",
    ...
    "",
    "  # 控制台 / 平台全局接口",
    `  location ~ ^${escapedPrefix}/(?<projectKey>[a-z0-9_-]+)/(?<env>[a-z0-9_-]+)/api/system/(?<systemPath>.+)$ {`,
    `    proxy_pass ${upstreamOrigin}/api/system/$systemPath;`,
    "    proxy_http_version 1.1;",
    "    proxy_set_header Host $host;",
    "    proxy_set_header X-Real-IP $remote_addr;",
    "    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;",
    ...
    "  }",
    "",
    `  location ~ ^${escapedPrefix}/(?<projectKey>[a-z0-9_-]+)/(?<env>[a-z0-9_-]+)/api/v2/(?<v2Path>.+)$ {`,
    `    proxy_pass ${upstreamOrigin}/api/v2/$v2Path;`,
    "    proxy_http_version 1.1;",
    "    proxy_set_header Host $host;",
    "    proxy_set_header X-Real-IP $remote_addr;",
    "    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;",
    ...
    "  }",
    "",
    `  location ~ ^${escapedPrefix}/(?<projectKey>[a-z0-9_-]+)/(?<env>[a-z0-9_-]+)/api/platform/(?<platformPath>.+)$ {`,
    `    proxy_pass ${upstreamOrigin}/api/platform/$platformPath;`,
    "    proxy_http_version 1.1;",
    "    proxy_set_header Host $host;",
    "    proxy_set_header X-Real-IP $remote_addr;",
    "    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;",
    ...
    "  }",
    "",
    `  location ~ ^${escapedPrefix}/(?<projectKey>[a-z0-9_-]+)/(?<env>[a-z0-9_-]+)/api/admin/(?<adminPath>.+)$ {`,
    `    proxy_pass ${upstreamOrigin}/api/admin/$adminPath;`,
    "    proxy_http_version 1.1;",
    "    proxy_set_header Host $host;",
    "    proxy_set_header X-Real-IP $remote_addr;",
    "    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;",
    ...
    "",
    "  # 保留原始网关直连能力，方便调试页面复用",
    `  location ~ ^${escapedPrefix}/(?<projectKey>[a-z0-9_-]+)/(?<env>[a-z0-9_-]+)/api/gw/(?<gatewayPath>.+)$ {`,
    `    proxy_pass ${upstreamOrigin}/api/gw/$gatewayPath;`,
    "    proxy_http_version 1.1;",
    "    proxy_set_header Host $host;",
    "    proxy_set_header X-Real-IP $remote_addr;",
    "    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;",
    ...
    "  }",
    "",
    "  # 以下接口绑定当前路径中的 project/env",
    `  location ~ ^${escapedPrefix}/(?<projectKey>[a-z0-9_-]+)/(?<env>[a-z0-9_-]+)/api/auth/me$ {`,
    `    proxy_pass ${upstreamOrigin}/api/gw/$projectKey/$env/auth/me;`,
    "    proxy_http_version 1.1;",
    "    proxy_set_header Host $host;",
    "    proxy_set_header X-Real-IP $remote_addr;",
    "    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;",
    ...
    "  }",
    "",
    `  location ~ ^${escapedPrefix}/(?<projectKey>[a-z0-9_-]+)/(?<env>[a-z0-9_-]+)/api/sql$ {`,
    `    proxy_pass ${upstreamOrigin}/api/gw/$projectKey/$env/sql;`,
    "    proxy_http_version 1.1;",
    "    proxy_set_header Host $host;",
    "    proxy_set_header X-Real-IP $remote_addr;",
    "    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;",
    ...
    "  }",
    "",
    `  location ~ ^${escapedPrefix}/(?<projectKey>[a-z0-9_-]+)/(?<env>[a-z0-9_-]+)/api/health$ {`,
    `    proxy_pass ${upstreamOrigin}/api/gw/$projectKey/$env/health;`,
    "    proxy_http_version 1.1;",
    "    proxy_set_header Host $host;",
    "    proxy_set_header X-Real-IP $remote_addr;",
    "    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;",
    ...
    "  }",
    "",
    `  location ~ ^${escapedPrefix}/(?<projectKey>[a-z0-9_-]+)/(?<env>[a-z0-9_-]+)/api/(?<apiKey>.+)$ {`,
    `    proxy_pass ${upstreamOrigin}/api/gw/$projectKey/$env/api/$apiKey;`,
    "    proxy_http_version 1.1;",
    "    proxy_set_header Host $host;",
    "    proxy_set_header X-Real-IP $remote_addr;",
    "    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;",
    ...
    "  }",
    "",
    "  # 共享路径下的静态资源与 SPA history fallback",
    `  location ~ ^${escapedPrefix}/(?<projectKey>[a-z0-9_-]+)/(?<env>[a-z0-9_-]+)/(?<assetPath>.*)$ {`,
    `    root ${webRootTemplate.replaceAll("{projectKey}", "$projectKey").replaceAll("{env}", "$env")};`,
    "    rewrite ^ /$assetPath break;",
...
function buildDefaultNginxSettings(projectKey, env) {
  ...
  return {
    ...
    pathBase: buildProjectPathBase(normalizedProject, normalizedEnv),
...
async function ensureSharedPathRoutingConfig() {
  if (!nginx.pathRoutingEnabled) {
    return null;
  }

  await ensureNginxConfDir();
  const confPath = resolveSharedPathRoutingConfPath();
  const configText = buildSharedPathRoutingConfigText();
...
function upgradeLegacyUpstreamOrigin(configText, upstreamOrigin) {
  const text = String(configText || "");
  const nextUpstream = normalizeUpstreamOrigin(upstreamOrigin);
  const legacyUpstream = "http://easydb-gateway:3000";
  if (!text.includes(legacyUpstream) || text.includes(nextUpstream)) {
    return {
      changed: false,
      configText: text,
    };
  ...
  return {
    changed: true,
    configText: text.replaceAll(legacyUpstream, nextUpstream),
  };
}
...
async function upsertProjectEnvNginxConfig(projectKey: string, env: string, payload: any = {}) {
  if (!nginx.enabled) {
    throw new Error("nginx config management is disabled");
  }
  const defaults = buildDefaultNginxSettings(projectKey, env);
  ...
  const sharedPathRouting = await ensureSharedPathRoutingConfig();
  ...
  return {
    ...
    pathRoutingConfPath: sharedPathRouting?.path || null,
    configText: normalizedText,
  };
}

async function ensureProjectEnvNginxConfig(projectKey: string, env: string, options: any = {}) {
  ...
  if (exists && !options.overwrite) {
    ...
    let upgradedLegacyUpstream = false;
    if (options.autoUpgradeLegacyRoot !== false) {
      const currentText = await fs.readFile(confPath, "utf8");
      const upgradedRoot = upgradeLegacyFrontendRoot(currentText, defaults.frontendRoot);
      const upgradedUpstream = upgradeLegacyUpstreamOrigin(
        upgradedRoot.configText,
        defaults.upstreamOrigin
      );
      if (upgradedRoot.changed || upgradedUpstream.changed) {
        const finalText = upgradedUpstream.configText;
        const normalizedText = finalText.endsWith("\n") ? finalText : `${finalText}\n`;
        await fs.writeFile(confPath, normalizedText, "utf8");
        upgradedLegacyRoot = upgradedRoot.changed;
        upgradedLegacyUpstream = upgradedUpstream.changed;
      }
    }
