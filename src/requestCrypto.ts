...
function resolveEncryptionOptions(
  options: Partial<RequestEncryptionOptions> = {}
): RequestEncryptionOptions {
...
function decryptPayload(
  ...
  const encrypted = normalized as Record<string, unknown>;
  const iv = toBuffer(encrypted.iv, "iv");
  const tag = toBuffer(encrypted.tag, "tag");
  const data = toBuffer(encrypted.data, "data");
...
function readRequestPayload(body: any, options: Partial<RequestEncryptionOptions> = {}) {
  const encryptionOptions = resolveEncryptionOptions(options);
