...
function parseRoleTableRules(roleTables: any) {
  if (!roleTables) return new Map();
  if (roleTables instanceof Map) return roleTables;

  const roleMap = new Map();
  for (const [roleName, rule] of Object.entries(roleTables as Record<string, any>)) {
    ...
    const sourceList = Array.isArray(rule)
      ...
      : typeof rule === "string"
        ? rule.split("|")
        : Array.isArray((rule as any)?.tables)
          ? (rule as any).tables
          : [];
...
function normalizeTable(tableToken: unknown) {
...
function validateSqlWithPolicy(sql: string, context: any = {}, policy: any = {}) {
...
function validateSql(sql: string, context: any = {}) {
