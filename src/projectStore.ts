...
function parsePolicyJson(raw: any) {
...
function buildProjectEnvContext(row: any) {
  if (!row) return null;
  return {
    envId: row.env_id,
...
async function listProjectEnvs(projectKey: string) {
  const project = await getProject(projectKey);
  if (!project) return [];
...
async function findProjectEnvRecord(projectKey: string, envKey: string) {
...
async function getProjectEnvContext(projectKey: string, envKey: string) {
  const row = await findProjectEnvRecord(projectKey, envKey);
  return buildProjectEnvContext(row);
}

async function upsertProjectEnv(projectKey: string, envKey: string, payload: any = {}) {
  const normalizedProjectKey = normalizeProjectKey(projectKey);
  const normalizedEnvKey = normalizeEnvKey(envKey);
...
async function listProjectEnvVars(projectKey: string, envKey: string, options: any = {}) {
...
async function upsertProjectEnvVar(
  projectKey: string,
  envKey: string,
  varKey: string,
  payload: any = {}
