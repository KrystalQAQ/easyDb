...
function UserCenterPage() {
  ...
  const loadUsers = useCallback(async () => {
    setLoading(true)
    try {
      const payload = await request('/api/v2/admin/users?limit=200')
  ...
  const onCreate = async () => {
    try {
      ...
      setCreating(true)
      await request('/api/v2/admin/users', {
        method: 'POST',
        body: {
  ...
  const onResetPassword = async () => {
    try {
      ...
      setResetting(true)
      await request(`/api/v2/admin/users/${encodeURIComponent(resetTarget)}/reset-password`, {
        method: 'POST',
