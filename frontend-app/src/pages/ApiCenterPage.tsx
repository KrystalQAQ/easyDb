...
function ApiEditorDrawer({ open, onClose, onSaved, editingApi, groups, projectKey, env, request }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingApi) {
        await request(`/api/v2/projects/${projectKey}/envs/${env}/apis/${editingApi.apiKey}`, {
          method: 'PUT',
          body,
        })
        message.success('接口已更新')
      } else {
        await request(`/api/v2/projects/${projectKey}/envs/${env}/apis`, {
          method: 'POST',
          body,
        })
  ...
  const run = async () => {
    ...
    try {
      const data = await request(
        `/api/v2/projects/${projectKey}/envs/${env}/apis/${api.apiKey}/test`,
...
function GroupModal({ open, onClose, onSaved, editingGroup, projectKey, env, request }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingGroup) {
        await request(
          `/api/v2/projects/${projectKey}/envs/${env}/api-groups/${editingGroup.groupKey}`,
          { method: 'PUT', body },
        )
        message.success('分组已更新')
      } else {
        await request(`/api/v2/projects/${projectKey}/envs/${env}/api-groups`, {
          method: 'POST',
          body,
        })
...
function ApiCenterPage() {
  ...
  const loadGroups = useCallback(async () => {
    try {
      const data = await request(`/api/v2/projects/${projectKey}/envs/${env}/api-groups`)
  ...
  const loadApis = useCallback(async () => {
    ...
    try {
      ...
      const data = await request(`/api/v2/projects/${projectKey}/envs/${env}/apis${qs}`)
  ...
  const handleDeleteApi = async (apiKey) => {
    try {
      await request(`/api/v2/projects/${projectKey}/envs/${env}/apis/${apiKey}`, { method: 'DELETE' })
  ...
  const handleDeleteGroup = async (groupKey) => {
    try {
      await request(`/api/v2/projects/${projectKey}/envs/${env}/api-groups/${groupKey}`, { method: 'DELETE' })
...
function ApiKeyPanel({ projectKey, env, request }) {
  ...
  const loadKeys = useCallback(async () => {
    setLoading(true)
    try {
      const data = await request(`/api/v2/projects/${projectKey}/envs/${env}/api-keys`)
  ...
  const handleRevoke = async (id) => {
    try {
      await request(`/api/v2/projects/${projectKey}/envs/${env}/api-keys/${id}/revoke`, { method: 'PUT' })
      message.success('已吊销')
      void loadKeys()
    } catch (err) {
      message.error(err.message)
  ...
  const handleDelete = async (id) => {
    try {
      await request(`/api/v2/projects/${projectKey}/envs/${env}/api-keys/${id}`, { method: 'DELETE' })
      message.success('已删除')
      void loadKeys()
    } catch (err) {
      message.error(err.message)
