...
import { buildApiUrl, detectRuntimeAccess } from '../lib/http'
...
function buildPathModeBase(pathBase) {
  const normalizedPathBase = String(pathBase || '').trim().replace(/\/+$/, '')
  if (!normalizedPathBase) return ''
  if (typeof window === 'undefined') return normalizedPathBase
  return `${window.location.origin}${normalizedPathBase}`
}

function buildPreferredAccessBase({ preferPathAccess, domainBase, pathBase }) {
  if (preferPathAccess) {
    return pathBase || domainBase || ''
  }
  return domainBase || pathBase || ''
}

function ProjectCenterPage() {
  const { request, token, projectKey, env, updateGatewayContext } = useConsole()
  const runtimeAccess = useMemo(() => detectRuntimeAccess(), [])
  const runtimeMode = runtimeAccess.mode
  const preferPathAccess = Boolean(runtimeAccess.preferPathAccess)
  const fixedEnv = 'prod'
  ...
  const [currentPathBase, setCurrentPathBase] = useState('')
    ...
    async (projectList) => {
      ...
      const pairs = await Promise.all(
        projectList.map(async (projectItem) => {
          try {
            const envPayload = await request(`/api/v2/projects/${encodeURIComponent(projectItem.projectKey)}/envs`)
            ...
            const nginxPayload = await request(
              `/api/v2/projects/${encodeURIComponent(projectItem.projectKey)}/envs/${encodeURIComponent(targetEnv)}/nginx`,
            )
            const settings = nginxPayload.item?.settings || {}
            const domainBase = buildDomainBase(settings.serverName)
            const pathBase = buildPathModeBase(settings.pathBase)
            const accessBase = buildPreferredAccessBase({ preferPathAccess, domainBase, pathBase })
            if (!accessBase) return null
            return [
    ...
    async (targetProject, targetEnv) => {
      ...
      try {
        const payload = await request(
          `/api/v2/projects/${encodeURIComponent(targetProject)}/envs/${encodeURIComponent(targetEnv)}`,
        )
  ...
  const loadNginx = useCallback(
    async (targetProject, targetEnv) => {
      ...
      try {
        const payload = await request(
          `/api/v2/projects/${encodeURIComponent(targetProject)}/envs/${encodeURIComponent(targetEnv)}/nginx`,
        )
        const item = payload.item || {}
        ...
        setCurrentPathBase(buildPathModeBase(item.settings?.pathBase))
      } catch (err) {
        setCurrentDomainBase('')
        setCurrentPathBase('')
        message.error(err.message)
  ...
  const onTestConnection = async () => {
    ...
    setConnTestResult(null)
    try {
      const result = await request('/api/v2/test-db-connection', {
        method: 'POST',
        body: {
          host: db.host,
    ...
    (targetProject) => {
      Modal.confirm({
        ...
        onOk: async () => {
          ...
          try {
            await request(`/api/v2/projects/${encodeURIComponent(targetProject)}`, {
              method: 'DELETE',
            })
  ...
  const onSaveEnv = async () => {
    ...
    try {
      ...
      setEnvSaving(true)
      await request(`/api/v2/projects/${encodeURIComponent(projectKey)}/envs/${encodeURIComponent(fixedEnv)}`, {
        method: 'PUT',
        body: {
          status: values.status,
          db: {
  ...
  const onSaveNginx = async () => {
    ...
    try {
      setNginxSaving(true)
      await request(
        `/api/v2/projects/${encodeURIComponent(projectKey)}/envs/${encodeURIComponent(fixedEnv)}/nginx`,
        {
          method: 'PUT',
          body: {
  ...
  const onReloadNginx = async () => {
    ...
    try {
      ...
      const payload = await request(
        `/api/v2/projects/${encodeURIComponent(projectKey)}/envs/${encodeURIComponent(fixedEnv)}/nginx/reload`,
        {
          method: 'POST',
          body: {},
        },
  ...
  const projectColumns = useMemo(
    () => [
      ...
      {
        title: preferPathAccess ? '内网访问地址' : '访问域名',
        render: (_, row) => {
          const domainItem = projectDomainMap[row.projectKey]
          if (!domainItem?.accessBase) return '-'
          return (
            <a href={domainItem.accessBase} target="_blank" rel="noreferrer">
              {domainItem.accessBase}
            </a>
          )
        },
      },
    ...
    [deletingProject, onDeleteProject, preferPathAccess, projectDomainMap],
  )

  const projectApiInfo = useMemo(() => {
    const domainBase = String(currentDomainBase || '').replace(/\/+$/, '')
    const pathBase = String(currentPathBase || '').replace(/\/+$/, '')
    const preferredBase = buildPreferredAccessBase({ preferPathAccess, domainBase, pathBase })
    if (!preferredBase) {
      return {
        preferredLabel: preferPathAccess ? '当前推荐访问地址（内网）' : '项目域名',
        preferredBase: '-',
    ...
    return {
      ...
      domainBase: domainBase || '-',
      pathBase: pathBase || '-',
      gatewayBase: `${preferredBase}/api`,
      login: `${preferredBase}/api/v2/auth/login`,
      me: `${preferredBase}/api/v2/auth/me`,
      sql: `${preferredBase}/api/sql`,
      health: `${preferredBase}/api/health`,
    }
  }, [currentDomainBase, currentPathBase, preferPathAccess])
            ...
            customRequest={async ({ file, onSuccess, onError }) => {
              ...
              try {
                const res = await fetch(
                  buildApiUrl(`/api/v2/projects/${encodeURIComponent(projectKey)}/envs/${encodeURIComponent(fixedEnv)}/deploy`),
      ...
      <Card>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <Form layout="vertical">
            <Form.Item label={projectApiInfo.preferredLabel}>
              <Input
                readOnly
                value={projectApiInfo.preferredBase}
                addonAfter={
                  <Button
                    size="small"
                    type="text"
                    icon={<CopyOutlined />}
                    onClick={() => void copyText(projectApiInfo.preferredBase, projectApiInfo.preferredLabel)}
                  >
                    复制
                  </Button>
                }
              ...
              <Input
                ...
                addonAfter={
                  <Button
                    ...
                    type="text"
                    icon={<CopyOutlined />}
                    onClick={() => void copyText(projectApiInfo.domainBase, '域名访问地址')}
                  >
                    复制
                  </Button>
                }
            ...
            <Form.Item label="内网路径地址">
              <Input
                readOnly
                value={projectApiInfo.pathBase}
                addonAfter={
                  <Button
                    size="small"
                    type="text"
                    icon={<CopyOutlined />}
                    onClick={() => void copyText(projectApiInfo.pathBase, '内网路径地址')}
                  >
                    复制
                  </Button>
                }
